...
def getData():
    try:
        ...
        comments = browser.find_elements(By.CLASS_NAME, 'chat-line__message')
        print("采集数据")
        commentsList_ = []
    ...
    try:
        if len(data) == 0:
            ...
            sql = f"insert into spider_abroad(username,comment,washtime,url)values {values}"
            print(sql)
